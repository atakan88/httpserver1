...
def on_server_side_rpc_request(request_id, request_body):
    ...
    if request_body["method"] == "TriggerShutdown2":
    ...
    if request_body["method"] == "TriggerReboot2":
        # Execute the command to shut down Ubuntu VM
        try:
            # Use subprocess to execute the shutdown command
...
try: 
    ...
    # Her 5 saniyede bir çalışacak kod bloğu
        ...
        # Get disk usage statistics
            ...
            #msg1 = json.dumps({"CPU_name2": cpu_brand})
                msg2 = json.dumps({"CPU2": cpu_usage})
                msg3 = json.dumps({"Memory2": memory_usage})
                msg4 = json.dumps({"Disk2": usage.percent})
                msg5 = json.dumps({"DiskTotal2": bytes_to_readable(usage.total)})
                msg6 = json.dumps({"DiskWriting2": bytes_to_readable(write_speed)})
                msg7 = json.dumps({"DiskReading2": bytes_to_readable(read_speed)})

            # Get IP location
            
                msg8 = '{"locationX2":"' + str(konum.lat) + '"}'
                msg9 = '{"locationY2":"' + str(konum.lng) + '"}'
    ...
    # Her 30 saniyede bir çalışacak kod bloğu
        ...
        # Check if the process has an associated executable path
                    ...
                    if exe_path:
                #print(f"running program: {process.name()}, {exe_path}")
            ...
            for i in range(total_count):
                ...
                msgo = json.dumps({"Running Exe 2": message})
